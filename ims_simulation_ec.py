...
P = {'current':{},'fqueue':[],'rqueue':[]}
S = {'current':{},'fqueue':[],'rqueue':[]}
AS = {'current':{},'fqueue':[],'rqueue':[]}
...
#			print "Going towards S-CSCF"			
			if(len(S['current'])==0):
				S['current'] = P['current']
				if (len(P['rqueue'])!=0):			
					P['current']=P['rqueue'].pop(0)
					next_time['tpc'] = MC - up*m.log(random.random())								
				elif (len(P['fqueue'])!=0):			
					P['current']=P['fqueue'].pop(0)
					next_time['tpc'] = MC - up*m.log(random.random())								
				else:
					P['current']={}
...
#		print S['current']
		if S['current']['flag'] is False:	#Using Flag is False for forward direction (P-CSCF --> S-CSCF --> AS)
			if(len(AS['current'])==0):
				...
				if (len(S['rqueue'])!=0):			
					S['current']=S['rqueue'].pop(0)
					next_time['tsc'] = MC - us*m.log(random.random())
				elif (len(S['fqueue'])!=0):			
					S['current']=S['fqueue'].pop(0)
					next_time['tsc'] = MC - us*m.log(random.random())
				else:
					S['current']={}
